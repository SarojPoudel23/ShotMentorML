...
class poseDetector():
    ...
    def findPose(self, img, draw = True):
        ...
        drawn_frame = np.zeros_like(img, dtype=np.uint8)
            ...
            if draw:
                ...
                for connection in connections:
                    try:
                        ...
                        cv2.line(drawn_frame, start_point, end_point, (255, 255, 255), 2)
                    except IndexError as err:
                        print(str(err))

                for id, lm in enumerate(landmark_points):
                    cv2.circle(drawn_frame, lm, 5, (255, 255, 255), cv2.FILLED)
        return drawn_frame